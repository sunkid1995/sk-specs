...
When this skill is activated (via `/feature`), execute the Feature Development Workflow:

1. Verify if `ba.md` exists and is approved under `.agents/sk-specs/active/<work-item-name>/`.
   - If `ba.md` does not exist, fall back to executing the `/ba` workflow first.
2. Automatically generate or update the `feature.md` file in `.agents/sk-specs/active/<work-item-name>/` using the format specified in `.agents/sk-specs/templates/feature.md`.
